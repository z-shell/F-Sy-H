...
```ini
CONFIG:    = ~/.config/fsh (respects $XDG_CONFIG_HOME env var)
CACHE:    = ~/.cache/zi/fsh (respects $XDG_CACHE_HOME env var)
LOCAL:  = /usr/local/share/fsh/
HOME:   = ~/.fsh/
OPT:    = /opt/local/share/fsh/
```

So for example, issue `fast-theme CONFIG:overlay` to load `~/.config/fsh/overlay.ini` as overlay. The `.ini`
...
You can use "~" in the path, e.g. `FAST_WORK_DIR=~/.fsh` and also the `CONFIG:`, `CACHE:`, `LOCAL:`, `OPT:`,
