...
The `KIND` column distinguishes command-specific `dedicated` handlers from the shared `generic` fallback. The generic handler paints the first operand as a subcommand only when that is unambiguous: the operand is a plain word and is preceded by no option, or only by options with an attached value (`--name=value`, `-n=value`). After an option without an attached value it paints nothing rather than guess, because without an option table a boolean flag cannot be told from one that consumes the next word; `systemctl --user restart foo` and `sudo apt -y install x` therefore paint no subcommand, while `kubectl -n ns get pods` no longer paints `ns`. A first operand that is a path, a number, or another non-word is not painted either. It does not validate the subcommand or parse command-specific options, so a generic registry entry does not represent the same depth of coverage as a dedicated handler. The shared precommand handler (`-`, `nohup`, `xargs`, `strace`, `ltrace`, `systemd-run`) applies the same rule to the command it runs: the first operand is painted as the command, options with an attached value are skipped, the word after `--` is the command whatever its shape, and after any other option nothing is painted as the command. `nohup cmd` and `systemd-run --unit=x cmd` paint `cmd`, while `strace -o out.txt cmd` and `strace -f cmd` leave the line neutral. `STATUS` reports whether the target is `ready`, `missing`, or intentionally `disabled`.

`hub` and `lab` use generic coverage; their older dedicated sources are retired. The unmaintained `zmanage` mapping is removed. Verb-first developer tools such as `gh`, `kubectl`, `cargo`, `uv`, and `terraform` are registered generically. Pattern-first tools (`rg`, `fd`, `journalctl`) and flag-driven `pacman` are intentionally not registered, because the generic handler would paint their first operand as a subcommand. Git repository queries and manual page lookups use the shared asynchronous worker. Tokens remain neutral until valid knowledge arrives and the next key press repaints the line; the worker callback never runs a widget between key presses, so `LASTWIDGET` and the kill and yank state stay untouched. Failed refreshes preserve the last valid result. Makefile variable expansion and static local `include`, `-include`, and `sinclude` discovery stay in the current shell without subprocesses. A refresh parses at most 16,384 characters across 32 files and eight include levels. Dynamic, unsupported, cyclic, missing mandatory, or over-budget input skips target validation so highlighting remains responsive and does not report false errors. Cache entries are scoped to the selected Makefile and invalidated when an included input changes.
