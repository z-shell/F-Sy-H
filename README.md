...
The `KIND` column distinguishes command-specific `dedicated` handlers from the shared `generic` fallback. The generic handler paints the first operand as a subcommand only when that is unambiguous: the operand is a plain word and is preceded by no option, or only by `--name=value` options. After an option that might consume the next word, or when the first operand is a path, a number, or another non-word, it paints nothing rather than guess. It does not validate the subcommand or parse command-specific options, so a generic registry entry does not represent the same depth of coverage as a dedicated handler. `STATUS` reports whether the target is `ready`, `missing`, or intentionally `disabled`.
