...
_fsh_highlight_string_process() {
  ...
  while [[ $_mybuf = (#b)([^"{}()[]\\\"'"]#)((["({[]})\"'"])|[\\](*))(*) ]]; do
    if [[ -n ${match[4]} ]] {
      ...
      __idx+=${mbegin[2]}
      [[ -z $__quoting && -z ${_fsh_complex_brackets[(r)$((__idx-${#PREBUFFER}-1))]} ]] && {
        if [[ ${match[2]} = ["({["] ]]; then
  ...
  for __idx in ${(k)pos_to_level}; do
    (( ${+final_pairs[$__idx]} )) && \
      __style=${_fsh_theme_name}bracket-level-$(( ( (pos_to_level[$__idx]-1) % 3 ) + 1 )) || \
  ...
  if [[ $WIDGET != zle-line-finish ]]; then
    ...
    if (( ${+pos_to_level[$__idx]} )) && (( ${+final_pairs[$__idx]} )); then
      (( __start=final_pairs[$__idx]-${#PREBUFFER}-1, __end=final_pairs[$__idx]-${#PREBUFFER}, __start >= 0 )) && \
        reply+=("$__start $__end ${_fsh_styles[${_fsh_theme_name}paired-bracket]}") && \
        reply+=("$CURSOR $__idx ${_fsh_styles[${_fsh_theme_name}paired-bracket]}")
    fi
  fi
  return 0
}
